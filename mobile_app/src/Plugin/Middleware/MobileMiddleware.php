...
namespace Simp\Pindrop\Modules\mobile_app\src\Plugin\Middleware;
...
class MobileMiddleware implements Middleware
{
    public function __invoke(Request $request, Access $access_interface, $next)
    {
        ...
        if ($currentUser === null) {
            ...
            $access_interface->redirect = new RedirectResponse(Url::routeByName('mobile_app.user.login'));
            return $next($request, $access_interface);
        }
        ...
        if (empty($permitted_roles)) {
            if (!$user || !$user?->getId()) {
                ...
                $access_interface->access_granted = false;
                $access_interface->redirect = new RedirectResponse(Url::routeByName('mobile_app.home'));
                return $next($request, $access_interface);
            }
        }
